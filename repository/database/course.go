...
func (r *Repository) CreateCourse(ctx context.Context, schoolID string, input *model.NewCourse) (course *model.Course, err error) {
	...
	sql := `INSERT INTO courses (name, code, school, taughtby) VALUES ($1, $2, $3, $4) RETURNING id, name, code, school, taughtby`
	err = r.DatabasePool.QueryRow(ctx, sql, input.Name, input.Code, schoolID, nil).Scan(&newCourse.ID, &newCourse.Name, &newCourse.Code, &newCourse.School, &newCourse.TaughtBy)
	if err != nil {
		return nil, err
	}
...
func (r *Repository) GetCourseById(ctx context.Context, id string) (course *model.Course, err error) {
	...
	sql := `SELECT id, name, code, school, taughtby FROM courses WHERE id = $1`
	err = r.DatabasePool.QueryRow(ctx, sql, id).Scan(&course1.ID, &course1.Name, &course1.Code, &course1.School, &course1.TaughtBy)
	if err != nil {
		return nil, err
	}
	return &course1, nil
